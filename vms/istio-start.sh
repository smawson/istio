...
if [[ -r ${ISTIO_CLUSTER_CONFIG} ]]; then
  ...
  export ISTIO_CP_AUTH ISTIO_SERVICE_CIDR ISTIO_INBOUND_PORTS CA_PROVIDER CA_ADDR PLUGINS GKE_CLUSTER_URL
  export SERVICE_ACCOUNT PILOT_CERT_PROVIDER JWT_POLICY TRUST_DOMAIN
fi
...
exec su -s /bin/bash -c "ISTIO_META_MESH_ID=proj-577052008081 ISTIO_META_WORKLOAD_NAME=productcatalogservice-vm ISTIO_META_OWNER=//compute.googleapis.com/projects/sven-asm-vms/us-west3-a/instances/vm-ig-zprn INSTANCE_IP=${ISTIO_SVC_IP} POD_NAME=${POD_NAME} POD_NAMESPACE=${NS} exec ${ISTIO_BIN_BASE}/pilot-agent proxy ${ISTIO_AGENT_FLAGS_ARRAY[*]} \
    --serviceCluster $SVC \
    --discoveryAddress ${PILOT_ADDRESS} \
    ${CONTROL_PLANE_AUTH_POLICY[*]} \
