...
const (
	K8sServiceType = "k8s.io/service"
)

// Controller manages the workload entry state.
...
// NewController returns a pointer to a newly constructed Controller instance.
func NewController(sharedInformers informers.SharedInformerFactory) (*Controller, error) {
	serviceInformer := sharedInformers.Core().V1().Services().Informer()
	c := &Controller{
		serviceInformer: serviceInformer,
	}

	// Need to watch services and workload entries. Let's just start with Services.
	// TODO(smawson): Also need to support ServiceEntry later.
